...
const count = (target, input) => {
  let int = 0;
  input.forEach(x => x.forEach(y => y === target ? int = int+1 : null));
  return int;
};
...
const totalSum = (input) => {
  let int = 0;
  input.forEach(x => x.forEach(y => int = int+y ));
  return int;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach((x,i) => {
    newArr.push([]);
    x.map(y => y%5 === 0 ? typeof(y) == 'string' ? null:newArr[i].push(2**y):null)});
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  let str = '';
  let num = 0;
  data.forEach(x => {x.gender == 'male' || x.gender == 'female' ? num==0 ? (str = str+`${x.name}`, num++): str = str+` and ${x.name}`:null});
  return str;
};
...
let findShortest = (data) => {
  let num = 10000;
  let str='';
  data.forEach(x => Number(x.height)<num ? (num = Number(x.height), str=x.name):null)
  return str;
};
