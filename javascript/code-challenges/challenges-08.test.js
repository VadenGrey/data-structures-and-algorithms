...
const isCapitalized = (str) => {
  ...
  for (let i=0; i<y.length; i++) {
    if (!y[i].toLowerCase().includes(y[i])) {
      let splStr = y[i].split('');
      let testArr = [];
      for (let j=0; j<splStr.length; j++) {
        if (/[a-zA-Z]/g.test(splStr[j])) {
          testArr.push(splStr[j]);
        }
      }
      arr.push(testArr.join(''));
    }
  }
  return arr;
};
