...
const maxInArray = (arr) => {
  let highVal = 0;
  arr.reduce((y,z) => {z>highVal ? highVal=z: null;});
  return highVal;
};
...
const updateNumbers = (obj) => {
  return Object.entries(obj).map(x => x.join(': '));
};
...
const getHouses = (arr) => {
  let houses = [];
  arr.map((x,i) => houses.push(arr[i].house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let tru = 0;
  arr.map((x,i) => x.name === character ? tru = Object.keys(arr[i]).includes('children') : null);
  return tru;
};
