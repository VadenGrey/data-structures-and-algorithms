...
const sortStarWarsCharacters = (starWarsArr) => {
  let x = starWarsArr.sort((a,b) => {return a.height-b.height;});
  return x.reverse();
};
...
const howMuchPencil = (str) => {
  let result = [];
  let pushThing = str;
  result.push(pushThing);
  let pushArray = str.split('');
  while (pushArray.length>0) {
    pushArray.shift();
    let pushThing = pushArray.join('');
    result.push(pushThing);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for (let i=0; i<recipe.ingredients.length; i++) {
    ...
    let j=0;
    let stringStage = [];
    while (recipe.ingredients[i][y] !== undefined) {
      y++;
      if (recipe.ingredients[i][y] === ' ') {
        j++;
      }
      if (j === 2) {
        while (recipe.ingredients[i][y] !== undefined) {
          y++;
          stringStage.push(recipe.ingredients[i][y]);
        }
        break;
      }
    }
    result.push(stringStage.join(''));
  }
  return result;
};
