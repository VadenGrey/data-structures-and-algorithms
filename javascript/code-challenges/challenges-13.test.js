...
const longestString = (arr) => {
  if (arr.length>0) {
    let num = 0;
    let index = 0;
    arr.reduce((x,y,i) => y.length>num ? (num = y.length , index = i): null);
    return index;
  } else {
    return -1;
  }
...
const findHappiness = (arr) => {
  let newArr = [];
  arr.forEach(x => x.includes(':)') ? newArr.push(x):null);
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  let newArr = [];
  let con = [];
  arr.forEach(x => x.split('').forEach((y,i) => isFinite(y)?y===' '? null:i===x.split('').length-1?(con.push(y),newArr.push(con.join('')),con = []):con.push(y):null));
  return newArr;
};
...
const onlyOddChars = (str) => {
  let newStr = '';
  str.split('').forEach((x,i)=> i%2 === 1? newStr = newStr + x: null)
  return newStr;
};
...
const allHappy = (arr) => {
  let num = true;
  arr.forEach(x => x.includes(':)')?null:num=false);
  return num;
};
