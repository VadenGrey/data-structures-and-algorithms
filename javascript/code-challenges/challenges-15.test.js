...
const screenForNames = (arr) => {
  let newArr = [];
  arr.forEach(x => /^(Mr|Dr|Mrs|Ms)\.\s\w+$/g.test(x)?newArr.push(x):null);
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  let str = '';
  let num = 0;
  arr.forEach((x => {Number(x.mass) > Number(arr[0].mass) ? num===0 ? (str =`${x.name}`, num++): str = str+` - ${x.name}`:null;}));
  return str;
};
...
const detectTicTacToeWin = (board) => {
  ...
  let Xcount = 0;
  let boo = false;
  board.forEach(a => a.forEach(b => b!==''?b==='X'?Xcount=Xcount+1:Ocount=Ocount+1:null));
  Ocount>3||Xcount>3?boo=true:null;
  return boo;
};
